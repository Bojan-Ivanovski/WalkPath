export default class Path {
    ...
    isAlpha(letterToCheck : string){
       return /[A-Z]/.test(letterToCheck);
    }

    nextPath(previousChar : number[]){
        let directions : number[] = [];

        let left : boolean = this.x > 0 &&
        ...
        let right : boolean = this.x < (this.mainGrid[this.y].length - 1)  &&
            (this.mainGrid[this.y][this.x+1] === "-" || this.mainGrid[this.y][this.x+1] === "+" || this.isAlpha(this.mainGrid[this.y][this.x+1])) &&
            !(previousChar[0] == this.x+1 && previousChar[1] == this.y);
        let up : boolean = this.y < (this.mainGrid.length - 1) &&
    ...
    walk() {
        let previousChar : number[] = [-1,-1];
        let currentChar : string = this.mainGrid[this.y][this.x];
